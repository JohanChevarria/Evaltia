...
export default function DashboardMainPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (!(user as any).email_confirmed_at) {
        ...
        router.replace(`/auth/check-email${query}`);
        return;
      }
      ...
      // Si NO ha completado onboarding → preparamos overlay
      if (!(profile as any).onboarding_completed) {
        setNeedsOnboarding(true);

        // Cargar lista de cursos para el overlay
        const { data: courseRows } = await supabase
          .from("courses")
          .select("id, name, description")
          .order("name", { ascending: true });

        setAvailableCourses(courseRows || []);
      ...
      const courses: CourseProgress[] = (courseRows ?? []).map((c: any) => ({
        id: c.id,
        name: c.course_name,
      ...
      const globalCorrectas = courses.reduce(
        (acc, c) => acc + (c.correctas ?? 0),
        0
      );
      const globalIncorrectas = courses.reduce(
        (acc, c) => acc + (c.incorrectas ?? 0),
        0
      );
      ...
      const personalizedData: DashboardData = {
        ...
        globalTotal,
        courses,
        activePractice: undefined, // luego lo conectamos a practice_sessions
      };
  ...
  const globalPercent =
    data.globalTotal > 0
      ? Math.round((globalRespondidas / data.globalTotal) * 100)
      : 0;
  ...
  async function handleOnboardingSave() {
    ...
    if (profileError) {
      ...
      setOnboardingError("Ocurrió un error guardando tu selección.");
      setOnboardingSaving(false);
      return;
    }

    // 3) Crear progreso inicial en course_progress
    ...
    if (progressError) {
      ...
      // No lo bloqueamos, pero lo registramos
    }

    setOnboardingSaving(false);
    setNeedsOnboarding(false);
    // Recargamos el dashboard para que lea la nueva info
          ...
          <div className="flex-1 bg-white/80 backdrop-blur-md rounded-2xl shadow-sm p-5">
            ...
            {data.courses.length === 0 ? (
              <p className="text-xs text-gray-500">
                Aún no tienes cursos activos. Una vez completes tu selección de
                cursos, aparecerán aquí.
              </p>
            ) : (
              <div className="space-y-4">
                {data.courses.map((course) => {
                  const correctPercent =
                    course.total > 0
                      ? Math.round((course.correctas / course.total) * 100)
                      : 0;
                  const incorrectPercent =
                    course.total > 0
                      ? Math.round((course.incorrectas / course.total) * 100)
                      : 0;
          ...
          {data.activePractice ? (
            <>
              ...
              <p className="mt-2 text-[11px] text-gray-500">
                Solo puedes tener una práctica activa. Si inicias una nueva, esta
                se reemplazará automáticamente.
              </p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm">
          <div className="w-full max-w-3xl bg-white rounded-2xl shadow-xl p-6 md:p-8 space-y-6">
            ...
            <p className="text-sm text-slate-600 text-center max-w-xl mx-auto">
              Selecciona hasta <span className="font-semibold">3 cursos</span> para que
              Evaltia pueda personalizar tu panel, práctica y simulacros.
              Siempre podrás cambiar esta selección desde tu configuración.
            </p>
